...
int main ( int argc, char * argv[] )
{
    try
    {
        ...
        input_vector_param.setDescription( input_vector_param.description() + SL( "Used to generate align objects' positions with roofs." ) );
        ...
        input_raster_param.setDescription( input_raster_param.description() + SL ( "Used to acquire raster-to-world resolution. "
                                           "Raster is used to find actual buildings' roofs positions." ) );

        Args::Arg & output_param = arguments::get_output ();
        output_param.setDescription( output_param.description() + "It is saved into the a map named "
        ...
        Args::Arg & min_first_pos_weight_param = arguments::get_min_first_pos_weight ();

        Args::Arg & max_first_pos_deviation_param = arguments::get_max_first_pos_deviation ();

        Args::Help help;
        help.setAppDescription(
            SL( "Utility to align buildings' vectors with roof positions on raster. "
                "The aligning can be performed in automatic mode with no user interaction, manual mode with total results revision "
        ...
        cmd.addArg ( min_first_pos_weight_param );
        cmd.addArg ( max_first_pos_deviation_param );
        cmd.addArg ( help );
        ...
        value_helper < double > min_first_pos_weight_helper ( min_first_pos_weight_param.value() );
        if ( !min_first_pos_weight_helper.verify ( std::cerr, "STOP: First position minimum weight value is incorrect." ) )
            return 1;

        // Building model projection step value
        value_helper < double > max_first_pos_deviation_helper ( max_first_pos_deviation_param.value() );
        if ( !max_first_pos_deviation_helper.verify ( std::cerr, "STOP: First position minimum deviation value is incorrect." ) )
            return 1;
        ...
        const double min_first_pos_weight = min_first_pos_weight_helper.value();
        const double max_first_pos_deviation = max_first_pos_deviation_helper.value();
        ...
        rsai::roof_locator finder (
                                      ...
                                      , roof_position_walk
                                      , roof_variants
                                      , min_first_pos_weight
                                      , max_first_pos_deviation
